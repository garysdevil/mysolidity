...
contract FactoryDeploy1{
    ...
    function deployAccountContract(address _owner) payable external {
...
contract FactoryDeploy2{ 
    ...
    function deployAccountContract(address _owner, uint _salt) payable external {
        Account account = new Account{value: 1111, salt: bytes32(_salt)}(_owner);
        // 记录被部署合约生成的地址
        accountArr.push(account);
    }
    //  获取指定合约的地址
